...
async function handleSearch() {
    ...
    const response = await fetch('https://tav.app.n8n.cloud/webhook-test/botflix', {
        method: 'POST',
        headers: {
    ...
    //um filme de porradaria de herois
    if (data && data.results.length > 0) {
        ...
        moviesGrid.innerHTML += `<div class="movie-card">
									...
									<div class="movie-info">
										...
										<p class="movie-overview">${movie.overview || "Sem descrição"}</p>
										<p class="movie-rating">⭐ ${movie.vote_average.toFixed() || "N/A"} / 10</p>
									</div>
								</div>`
    }
